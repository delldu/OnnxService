...
enum GridSamplerInterpolation { Nearest = 1, Bilinear = 0, Bicubic = 2 };
...
template < typename scalar_t >
	static inline scalar_t grid_sampler_compute_source_index(scalar_t coord,
															 int64_t size, int64_t padding_mode, bool align_corners)
{
...
template < typename scalar_t >
	static inline scalar_t get_value_bounded(const scalar_t * data, scalar_t x,
											 ...
											 int64_t sW, int64_t sH, int64_t padding_mode, bool align_corners)
{
...
template < typename scalar_t > static inline scalar_t cubic_convolution1(scalar_t x, scalar_t A)
{
	return ((A + 2) * x - (A + 3)) * x * x + 1;
}

template < typename scalar_t > static inline scalar_t cubic_convolution2(scalar_t x, scalar_t A)
{
	return ((A * x - 5 * A) * x + 8 * A) * x - 4 * A;
}

template < typename scalar_t > static inline void get_cubic_upsample_coefficients(scalar_t coeffs[4], scalar_t t)
{
	scalar_t A = -0.75;
				...
				if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
					...
					for (int64_t c = 0; c < C; ++c, out_ptr_NCHW += out_sC, inp_ptr_NC += inp_sC) {
						...
						for (int64_t i = 0; i < 4; ++i) {
							coefficients[i] =
								cubic_interp1d < float >(get_value_bounded <
														 float >(inp_ptr_NC, ix_nw - 1, iy_nw - 1 + i, inp_W, inp_H,
																 inp_sW, inp_sH, padding_mode, align_corners),
														 get_value_bounded < float >(inp_ptr_NC, ix_nw + 0,
																					 iy_nw - 1 + i, inp_W, inp_H,
																					 inp_sW, inp_sH, padding_mode,
																					 align_corners),
														 get_value_bounded < float >(inp_ptr_NC, ix_nw + 1,
																					 iy_nw - 1 + i, inp_W, inp_H,
																					 inp_sW, inp_sH, padding_mode,
																					 align_corners),
														 get_value_bounded < float >(inp_ptr_NC, ix_nw + 2,
																					 iy_nw - 1 + i, inp_W, inp_H,
																					 inp_sW, inp_sH, padding_mode,
																					 align_corners), tx);
						}
