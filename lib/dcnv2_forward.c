...
DCNv2ForwardKernel::DCNv2ForwardKernel(OrtApi api, const OrtKernelInfo *info)
  ...
  // align_corners_ = ort_.KernelInfoGetAttribute < int64_t > (info,
  // "align_corners"); interpolation_mode_ = ort_.KernelInfoGetAttribute <
  // int64_t > (info, "interpolation_mode"); padding_mode_ =
  // ort_.KernelInfoGetAttribute < int64_t > (info, "padding_mode");
...
void DCNv2ForwardKernel::Compute(OrtKernelContext *context) {
  ...
  // *>(ort_.GetTensorData < float >(input));

  const OrtValue *grid = ort_.KernelContext_GetInput(context, 1);
  // const float *grid_data = reinterpret_cast < const float
  // *>(ort_.GetTensorData < float >(grid));
  ...
  // output_dims.data(), output_dims.size()); float *out_ptr =
