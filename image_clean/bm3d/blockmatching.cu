...
template < typename T > __device__ __inline__ T L2p2(const T i1, const T i2)
{
	T diff = i1 - i2;
	return diff * diff;
}
...
*/
__device__ void add_to_matched_image(uint * stack,	//IN/OUT: Stack of N patches matched to current reference patch
...
*/
__global__ void block_matching(const uchar * __restrict image,	//IN: Original image
							   ...
							   const uint2 start_point)	//IN: Address of the top-left reference patch of a batch
{
	...
	for (int i = threadIdx.x; i < p_rectangle_width * params.k; i += blockDim.x) {
		...
		if (p_rectangle_start + sx >= image_dim.x)
			continue;
	...
	for (int y = from.y; y <= (int) params.n; ++y) {
		...
		for (int x = from.x; x <= (int) params.n; x += num_warps) {
			...
			for (uint i = tid;
				 i < p_rectangle_width && p_rectangle_start + i < image_dim.x && q_rectangle_start + i < image_dim.x;
				 i += warpSize) {
				uint dist = 0;
				for (uint iy = 0; iy < params.k; ++iy) {
					dist +=
						L2p2((int) s_image_p[idx2(i, iy, p_rectangle_width)],
							 (int) image[idx2(q_rectangle_start + i, q.y + iy, image_dim.x)]);
				}
				s_diff[i] = dist;
			}

			if (p.x >= stacks_dim.x || q.x >= stacks_dim.x)
				continue;
			...
			if (diff < params.Tn) {
				...
				add_to_matched_image(&s_stacks[params.N * idx2(tid, wid, warpSize)],
									 &s_patches_in_stack[idx2(tid, wid, warpSize)], diff, params);
			}
		}
	}
...
extern "C" void run_block_matching(const uchar * __restrict image,	//Original image
...
{
	block_matching <<< num_blocks, num_threads, shared_memory_size >>> (image,
																		stacks,
																		num_patches_in_stack,
																		image_dim, stacks_dim, params, start_point);
}
