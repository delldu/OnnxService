...
//Extern kernels
extern "C" void run_block_matching(const uchar * __restrict image,
								   ushort * stacks,
								   ...
								   const dim3 num_threads, const dim3 num_blocks, const uint shared_memory_size);
...
extern "C" void run_get_block(const uint2 start_point,
							  ...
							  const uint2 image_dim,
							  const uint2 stacks_dim,
							  const Params params, const dim3 num_threads, const dim3 num_blocks);

extern "C" void run_DCT2D8x8(float *d_transformed_stacks,
							 const float *d_gathered_stacks,
							 const uint size, const dim3 num_threads, const dim3 num_blocks);

extern "C" void run_hard_treshold_block(const uint2 start_point,
										float *patch_stack,
										float *w_P,
										const uint * __restrict num_patches_in_stack,
										const uint2 stacks_dim,
...
extern "C" void run_aggregate_block(const uint2 start_point,
...
extern "C" void run_aggregate_final(const float *__restrict numerator,
									...
									const uint2 image_dim,
									uchar * denoised_noisy_image, const dim3 num_threads, const dim3 num_blocks);

extern "C" void run_wiener_filtering(const uint2 start_point,
									 float *patch_stack,
...
class BM3D {
  private:
	...
	void allocate_device_auxiliary_arrays() {
		...
		int maxN = std::max(h_wien_params.N, h_hard_params.N);
		 ...
		 cuda_error_check(cudaMalloc
						  ((void **) &d_num_patches_in_stack, sizeof(uint) * h_batch_size.x * h_batch_size.y));

		 cuda_error_check(cudaMalloc
						  ((void **) &d_gathered_stacks,
	...
	void copy_device_image(const uchar * src_image, int width, int height, int channels) {
		...
		for (int i = 0; i < channels; ++i) {
			...
			cuda_error_check(cudaMemcpy
							 (d_noisy_image[i], src_image + i * image_size, image_size * sizeof(uchar),
							  cudaMemcpyHostToDevice));
		}
	}

	//Compute launch parameters for block-matching kernel
	void get_BM_launch_parameters(const Params & params,	//IN: Denoising parameters
								  ...
								  uint & s_mem_size)	//OUT: shared memory size
	{
		...
		const uint num_warps =
			std::min(shared_mem_avaliable / (s_diff_size + s_patches_in_stack_size + s_patch_stacks_size), 32u);
		...
		if (_verbose) {
			std::cout << "Shared memory : " << s_mem_size / 1024 << "KB/" << properties.sharedMemPerBlock /
				1024 << "KB" << std::endl;
	...
	void first_step(std::vector < uchar * >&denoised_image, int width, int height, int channels, uint * sigma) {
		...
		const dim3 num_blocks_tr((trans_size + (KER2_BLOCK_WIDTH * h_hard_params.k) - 1) /
								 (KER2_BLOCK_WIDTH * h_hard_params.k), 1, 1);
		...
		for (start_point.y = 0; start_point.y < stacks_dim.y + h_hard_params.p - 1;
			...
			for (start_point.x = 0; start_point.x < stacks_dim.x + h_hard_params.p - 1;
				...
				run_block_matching(d_noisy_image[0],	// IN: Image
				...
				for (int channel = 0; channel < channels; ++channel) {
					...
					run_get_block(start_point,	//IN: First reference patch of a batch
					...
					run_hard_treshold_block(start_point,	//IN: First reference patch of a batch
					...
					run_aggregate_block(start_point,	//IN: First reference patch of a batch
		...
		for (int channel = 0; channel < channels; ++channel) {
			run_aggregate_final(d_numerator[channel],	//IN: Numerator aggregation buffer
	...
	void second_step(std::vector < uchar * >&denoised_image, int width, int height, int channels, uint * sigma) {
		...
		const dim3 num_blocks_tr((trans_size + (KER2_BLOCK_WIDTH * h_wien_params.k) - 1) /
								 (KER2_BLOCK_WIDTH * h_wien_params.k), 1, 1);
		...
		for (start_point.y = 0; start_point.y < stacks_dim.y + h_wien_params.p - 1;
			...
			for (start_point.x = 0; start_point.x < stacks_dim.x + h_wien_params.p - 1;
				 start_point.x += (h_batch_size.x * h_wien_params.p)) {
				if (_verbose)
					time_blockmatching.start();

				run_block_matching(d_denoised_image[0],	// IN: Image
								   d_stacks,	// OUT: Array of adresses of similar patches
				...
				for (int channel = 0; channel < channels; ++channel) {
					...
					run_DCT2D8x8(d_gathered_stacks_basic, d_gathered_stacks_basic, trans_size, num_threads_tr,
								 num_blocks_tr);
					cuda_error_check(cudaGetLastError());
					cuda_error_check(cudaDeviceSynchronize());
					 ...
					 */
					run_wiener_filtering(start_point,	//IN: First reference patch of a batch
	...
	void copy_host_image(uchar * dst_image, int width, int height, int channels) {
		size_t image_size = width * height;
		for (int channel = 0; channel < channels; ++channel) {
			cuda_error_check(cudaMemcpy(dst_image + channel * image_size,	// Destination
  ...
  BM3D():h_hard_params(),
	...
	BM3D(uint n, uint k, uint N, uint T, uint p, float L3D, bool seceon_step):h_hard_params(n, k, N, T, p, L3D),
	 ...
	 */
	void denoise_host_image(uchar * src_image, uchar * dst_image, int width, int height, int channels, uint * sigma,
							bool two_step) {
		Stopwatch total;
		total.start();

		//Allocation
		if (h_reserved_width != width || h_reserved_height != height || h_reserved_channels != channels
